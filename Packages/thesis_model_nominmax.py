...
class autocorrelation:
    ...
    def get_highest_power_spectrum(self, n_input):
        ...
        power_spectrum = abs((ifft(np.log10(abs(fft(sequence))**2)))**2)[1:][::-1]
        # get indexes from highest power_spectrum
        ### ORIGINAL###
        best_index_power_spectrum = np.sort(np.argpartition(power_spectrum, -n_input)[-n_input:])

        ### TESTE ###
        # best_index_power_spectrum = [-1,-2,-3, -25,-26,-27, -49,-50,-51]

        # min power spectrum value without first autocorrolation
        min_power_spectrum = power_spectrum[best_index_power_spectrum].max()
    ...
    def get_highest_acf(self, n_input):
        ...
        acf = abs(statsmodels.tsa.stattools.acf(sequence[::-1], nlags= len(sequence)))[1:][::-1]
        # get indexes from highest power_spectrum
        best_index_acf = np.sort(np.argpartition(acf, -n_input)[-n_input:])
        # min power spectrum value without first autocorrolation
        min_acf = acf[best_index_acf].max()
