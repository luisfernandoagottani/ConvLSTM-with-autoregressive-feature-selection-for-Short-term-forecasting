...
class thesis_sequence_algorithm:
    ...
    def master_algorithm(self):
        ...
        for i in range((self.warm_up_time),len(self.dataset)):
            ...
            if self.autocorrelation_type == "power_spectrum":
                ...
                #Testing
                #theta_list.append(max_power_spectrum)
                ...
                if max_power_spectrum > self.theta_threshold:
                    ...
                    #REV_01
                    #input_x = np.concatenate([train_x[max_index_power_spectrum][0][:1],train_x[-self.n_input:][0][1:]])

                    #REV_02
                    input_x = train_x[-self.n_input:]

                    #REV_03
                    #input_x = train_x[max_index_power_spectrum]
                ...
                dataset_autocorrelation = autocorrelation(train_x)
                max_power_spectrum, max_index_power_spectrum = dataset_autocorrelation.get_highest_acf(self.n_input)
                ...
                if max_power_spectrum > self.theta_threshold:
                    ...
                    #REV_03
                    # list_input = []
                    # train_x_selected = train_x[max_index_power_spectrum][:,:4]
                    # train_x_weather = train_x[-self.n_out:][:,4:]
                    # for i in range(0,len(train_x_selected)):
                    #     concat_train_x = np.concatenate([train_x_selected[0],train_x_weather[0]])
                    #     list_input.append(concat_train_x)
                    # input_x = np.array(list_input)

                    #REV_04 SAR_ConvLSTM with multiple enters
                    #input_x = train_x[max_index_power_spectrum]
    ...
    def master_algorithm_convlstm(self):
        ...
        for i in range((self.warm_up_time),len(self.dataset)):
            ...
            input_x = dataset_array[(i-self.n_input):i]
            # Reshape
            input_x = input_x.reshape((1, self.n_steps, self.n_length, self.n_features,1))
            # Predict
            y_hat = model.predict(input_x, verbose=0)
